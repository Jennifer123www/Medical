...
def MyModel(X_train,Y_train,X_test,Y_test,learning_rate = 0.001,epoch = 1000):
    '''
    ...
    w = np.zeros((X_train.shape[0],1))#应该都初始化为（nx,1） 因为后续操作 w叉乘x
...
if __name__ == '__main__':
    ...
    costs,learning_rate = MyModel(train_set_x,train_set_y,test_set_x,test_set_y)
    plt.plot(costs)
    plt.ylabel('cost')
