...
def MyModel(X_train,Y_train,X_test,Y_test,classes,learning_rate = 0.001,epoch = 1000,hidden_layers = 10):
    ...
    w1 = np.random.randn(hidden_layers,X_train_temp.shape[0])*0.01
    b1 = np.zeros((hidden_layers,1))
    # w1 = np.zeros((hidden_layers,X_train.shape[0]))
    # b1 = 0
    print('w1:',w1.shape,'b1:',b1.shape)
    # print(w1)
    w2 = np.random.randn(Y_train_temp.shape[0],hidden_layers)
    b2 = np.zeros((Y_train_temp.shape[0],1))
    # w2 = np.zeros((Y_train.shape[0], hidden_layers))
    # b2 = 0
    print('w2:',w2.shape,'b2:',b2.shape)
    # #-------------------训练--------------------------------
    m = X_train_temp.shape[1]  #m是样本数量
    print('m:',m)
    costs = []
    for i in range(epoch):
        ...
        logprobs = np.multiply(np.log(A2.T),Y_train2) + np.multiply((1-Y_train2),np.log(1-A2.T))
    ...
    #
     ...
     #-------------------------评估--------------------------------
    #*****训练集********
    Y_prediction_train  = predict(w1,b1,w2,b2,X_train_temp)
    print("训练集的准确性：",(100-np.mean(np.abs(Y_prediction_train-Y_train_temp))*100),'%')
    #****测试集********
    Y_prediction_test = predict(w1,b1,w2,b2,X_test_temp)
